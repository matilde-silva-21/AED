...
class BST {
    ...
    friend class BSTItrLevel<Comparable>;
    friend class iteratorBST<Comparable>;
    int depth(const Comparable& x, BinaryNode<Comparable> t, int count);
public:
...
template <class Comparable>
int BST<Comparable>::depth(const Comparable& x, BinaryNode<Comparable> t, int count){

    if(t == NULL)
        return -1;
    else if(x < t->element)
        return depth(x,t->left, count++);
    else if(t->element < x)
        return depth(x, t->right, count++);
    else
        return count;    // Match
